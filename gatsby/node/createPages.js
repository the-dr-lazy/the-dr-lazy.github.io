...
const templatesDirectory = path.resolve(__dirname, '../../src/Template')
const templates = {
    articles: path.resolve(templatesDirectory, 'Articles.tsx'),
    article: path.resolve(templatesDirectory, 'Article.tsx'),
...
function buildPaginatedPath(index, basePath) {
    return index > 1 ? path.join(basePath, 'page', `${index}`) : basePath
}
