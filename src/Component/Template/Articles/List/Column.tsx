...
export function component({ article, layout, narrow }: Props) {
    ...
    const image = hasHeroImage ? <Image.component src={imageSource} /> : <Image.Placeholder.component />
    ...
    return (
        <Link className={classNames.elements.link} to={article.slug} data-a11y="false">
            <div className={blockClassName}>
                <div className={classNames.elements.image}>{image}</div>
                <div>
                    <Heading.h2 className={classNames.elements.title}>{article.title}</Heading.h2>
