...
import { VDOM, arrayEmptyUnless, defineDisplayName, getPagePath, monoidVDOM } from '~/Data'
...
function mkPageNumberButton(basePath: string) {
    ...
    return ({ page, isActive }: Spec) => (
        <Link key={page} to={getPagePath(basePath, page)} style={{ opacity: isActive ? 1 : 0.3 }} className={classNames.elements.page}>
            {page}
        </Link>
    )
}

function mkPaginatorStructure(total: number, current: number, maxPages: number): Option<number>[] {
    ...
    return (
        <>
            ...
            <nav className={classNames.block}>
                ...
                {pageNumberButtons}
                <span className={classNames.elements.mobileReference} aria-hidden="true">
