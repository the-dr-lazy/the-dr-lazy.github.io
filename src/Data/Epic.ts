...
export function combineEpics(
  epicOrEpicsMap:
    ...
    | EpicsMap<AnyAction, AnyAction, {}, unknown>,
  ...args: Epic<AnyAction, AnyAction, unknown, unknown>[]
): Epic<AnyAction, AnyAction, unknown, unknown> {
  if (typeof epicOrEpicsMap === 'object') {
    return (action$, state$, environment) =>
      merge(
        ...Object.entries(epicOrEpicsMap).map(([key, epic]) =>
          epic(action$, state$.pipe(pluck(key)), environment),
        ),
      )
  }
  ...
  return (action$, state$, environment) =>
    merge(...epics.map(epic => epic(action$, state$, environment)))
}

export function mkRehydrateEpic<A>(
  get$: Observable<A>,
  mkAction: (a: A) => AnyAction,
) {
