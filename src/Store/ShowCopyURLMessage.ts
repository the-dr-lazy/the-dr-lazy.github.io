...
import { debounceTime, delayWhen, mapTo, pluck, switchMapTo } from 'rxjs/operators'
...
export const reducer = mkReducer(defaultState, handleAction => [handleAction(show, constant(true)), handleAction(hide, constant(false))])

//
// Epics
//

function copyURLToClipboardEpic(action$: Observable<AnyAction>): Observable<AnyAction> {
    return action$.pipe(
