...
export const prefer = mkActionCreator(
  '[Theme] set preferred',
  resolve => (theme: Theme) => resolve(theme),
)
...
function setClassEpic(
  _action$: Observable<unknown>,
  state$: Observable<State>,
): Observable<never> {
  return state$.pipe(
    distinctUntilChanged(),
    switchMap(Effect.DOM.setThemeClass$),
  )
}
...
function persistEpic(
  ...
  return action$.pipe(
    ...
    switchMapTo(state$),
    distinctUntilChanged(),
    switchMap(Effect.Storage.setTheme$(8 * 60 * 60 * 1000)),
  )
}
...
function resetStorageEpic(action$: Observable<AnyAction>): Observable<never> {
  return action$.pipe(
    ofType(prefer),
    skip(1),
    switchMapTo(Effect.Storage.removeTheme$),
  )
}
