import { EMPTY, Observable, animationFrameScheduler, combineLatest, defer, fromEvent, of, NEVER, merge } from 'rxjs'
import { distinctUntilChanged, map, observeOn, startWith, switchMap } from 'rxjs/operators'
...
export const setScrollPositionClass$ = setClass$(classNames.modifiers.scrollPosition)
...
export const scrollTop$: Observable<number> = (typeof window !== 'undefined' ? fromEvent(window, 'scroll', { passive: true }) : NEVER).pipe(
    observeOn(animationFrameScheduler),
    map(() => document.documentElement.scrollTop),
    startWith(typeof window !== 'undefined' ? document.documentElement.scrollTop : 0),
    distinctUntilChanged(),
)

export const scrollHeight$: Observable<number> = (typeof window !== 'undefined' ? fromEvent(window, 'resize', { passive: true }) : NEVER).pipe(
    observeOn(animationFrameScheduler),
    map(() => document.documentElement.scrollHeight),
    startWith(typeof window !== 'undefined' ? document.documentElement.scrollHeight : 768),
    distinctUntilChanged(),
)

export const innerHeight$: Observable<number> = (typeof window !== 'undefined' ? fromEvent(window, 'resize', { passive: true }) : NEVER).pipe(
    observeOn(animationFrameScheduler),
...
export const scrollPosition$: Observable<ScrollPosition> = combineLatest(
    ...
    scrollHeight$,
    innerHeight$,
    (scrollTop, scrollHeight, innerHeight) => (scrollTop / (scrollHeight - innerHeight) >= 0.5 ? ScrollPosition.Bottom : ScrollPosition.Top),
).pipe(distinctUntilChanged())

export const setScrollPosition$ = scrollPosition$.pipe(switchMap(setScrollPositionClass$))
