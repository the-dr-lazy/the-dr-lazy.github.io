...
function set$(key: Key, value: unknown, expiry: number): Observable<never> {
  return defer(() => {
    localStorage.setItem(
      key,
      JSON.stringify({ value, expiry, createdAt: Date.now() }),
    )
...
function get$<A>(key: Key, codec: t.Type<A>): Observable<A> {
  return defer(() => {
    ...
    const entity = pipe(
      ...
      O.fromNullable,
      O.chain(json(mkEntityC(codec))),
    )

    return Rx.fromOption(entity).pipe(
      mergeMap(entity =>
        isEntityExpired(entity) ? remove$(key) : of(entity.value),
      ),
    )
  })
}
